...
describe("createClient", () => {
    it("creates client with default api url and token change wiring", async () => {
        ...
        (globalThis as any).fetch = vi.fn(async () => new Response(JSON.stringify({ Entities: [] }), { status: 200 }));
        try {
            const client = createClient({ saleUUID: "sale", onTokenChange });
            await client.listAvailableEntities({ saleUUID: "sale" });
    ...
    it("respects custom apiURL and accepts provided AuthSession", async () => {
        ...
        const fetchSpy = vi.fn(async (input: RequestInfo | URL) => {
            ...
            expect(u.toString()).toBe(`${apiURL}/externalapi.ListAvailableEntities`);
        ...
        await expect(client2.listAvailableEntities({ saleUUID: "sale" })).rejects.toBeInstanceOf(Error);
